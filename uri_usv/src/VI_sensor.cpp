...
#define INA260_I2CADDR_2	0x41 ///< the second INA260
...
sensor_msgs::BatteryState batt_state2;
...
int main(int argc, char** argv)
{
  ...
  */
  ...
  // get the voltage and current data and publish it by the publisher
	...
	//when the battery is not supplying and device, if sometimes output large current reading
	if(batt_state.current>30)
	{
	...
	//sample the payload battery
	rc_i2c_set_device_address(1, INA260_I2CADDR_2);
        //// get the voltage and current data and publish it by the publisher
        rc_i2c_read_bytes(1, INA260_REG_CURRENT, 2, cu);
        //rc_i2c_set_device_address(1, INA260_I2CADDR_DEFAULT);
        rc_i2c_read_bytes(1, INA260_REG_BUSVOLTAGE, 2, vo);
        rc_i2c_read_bytes(1, INA260_REG_POWER, 2, po);
        //ROS_INFO("voltage0:%d\n voltage1:%d\n", int(vo[0]),int(vo[1]));
        batt_state2.voltage = 1.25*int((vo[0]<<8)|(vo[1]))/1000; //combine the high bits and the low bi$
        batt_state2.current = 1.25*int((cu[0]<<8)|(cu[1]))/1000;
	if(batt_state2.current>30)
        { 
                batt_state2.current =0;
        }
        power=10*int((po[0]<<8)|(po[1]))/1000;
        batt_state2.charge=batt_state.charge+power*1.00/3600.00;
        //node_priv.setParam("Wh", batt_state.charge);
        batt_pub2.publish(batt_state2);
